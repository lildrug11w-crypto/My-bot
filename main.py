...
# Базы данных в памяти (для примера; при перезапуске будут сбрасываться)
...
def handle_callbacks(call):
  ...
  # Обработка выбора конкретной модели
    ...
    m_key = data.split("_", 1)[1]  # достаем ключ модели
    if m_key in MODELS_CONFIG:
      ...
      # Возвращаем в главное меню с обновленной информацией
  ...
  # Раздел: Монеты и Ежедневная награда
    ...
    coins_text = (
        ...
        "• Приглашайте друзей, чтобы получать больше монет."
    )
    bot.edit_message_text(
        coins_text,
        chat_id=chat_id,
  ...
  # Получение ежедневного бонуса (+5 монет)
    ...
    # Проверка: прошло ли 24 часа (86400 секунд). Для теста можете временно уменьшить, если нужно.
  ...
  # Раздел: Рефералы
    ...
    bot_username = (
        bot.get_me().username
    )  # Автоматически получает юзернейм вашего бота
...
# Обработчик обычных текстовых сообщений (общение с нейросетью)
    ...
    try:
      ...
      with urllib.request.urlopen(req) as response:
        ...
        if "choices" in response_data and len(response_data["choices"]) > 0:
          ...
          bot.reply_to(
              message,
              f"{ai_reply}\n\n*(Списана 1 монета 🪙. Остаток: {user_coins[chat_id]}لت)*",
          )
          return
        else:
      ...
      if e.code == 429 and attempt < 2:
        ...
        # Возвращаем монету обратно при ошибке лимита
        user_coins[chat_id] += 1
        bot.reply_to(
            message,
